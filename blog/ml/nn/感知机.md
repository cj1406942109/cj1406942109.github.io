...
感知机的学习目标是求得一个能将训练集中的正实例点和负实例点完全正确分开的超平面。

为了找出这样的超平面，确定感知机模型的参数$w, \theta$，需要定义损失函数，并将损失函数极小化。

损失函数的一个自然选择是误分类点的总数，但是这样的函数不可导，难以优化。  
感知机采用的是另一个选择，误分类点到超平面的总距离。

下面，推导损失函数的计算公式：

设$M$为误分类的点的集合、感知机的模型参数$w, b$，输入空间中任一点$x_{0}$到超平面$S$的距离为：
$$ \frac{1}{\Vert w \Vert} \vert wx_{0} + b \vert $$
其中$\Vert w \Vert$是$w$的二范数。

并且，对于误分类数据$(x_{i}, y_{i})$：  
若$wx_{i} + b \gt 0$，则$y_{i} = -1$；若$wx_{i} + b \lt 0$，则$y_{i} = 1$；
所以有：$-y_{i}(wx_{i} + b) \gt 0$  
则，$-y_{i}(wx_{i} + b) = \vert wx_{i} + b \vert$

误分类点$x_{i}$到超平面$S$的距离为:
$$ -\frac{1}{\Vert w \Vert}y_{i}(wx_{i} + b)  $$

所有误分类点到超平面$S$的总距离为：
$$ -\frac{1}{\Vert w \Vert}\sum_{x_{i} \in M} y_{i}(wx_{i} + b)  $$

因为$\frac{1}{\Vert w \Vert}$不影响正负值的判断，将其忽略对损失函数没有影响，因此得到损失函数：
$$ L(w,b) = -\sum_{x_{i} \in M}y_{i}(wx_{i} + b) $$

采用随机梯度下降法对损失函数进行极小化，则损失函数的梯度为：
$$
\begin{cases}
    \Delta_{w} = -\sum_{x_{i} \in M} y_{i}x_{i}
    \\
    \Delta_{b} = -\sum_{x_{i} \in M} y_{i}
\end{cases}
$$

随机选取一个误分类点$(x_{i}, y_{i})$，对$w, b$进行更新的过程为：
$$
\begin{cases}
    w := w + \eta y_{i}x_{i}
    \\
...
将$b$看做一个固定输入为$-1.0$的“哑结点”所对应的连接权重$w_{n+1}$，则得到更新过程为：
$$ w := w + \eta y_{i}x_{i} $$
与上面提到的学习规则结果相同。
